        ...
        >
            ...
            >
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",
                    ...
                    <div>
                        <div style={{fontSize:14,color:"#6b7280"}}>{dateText}</div>
                        <div style={{fontSize:18,fontWeight:700}}>일정 {sorted.length}건</div>
                    </div>
                    <button onClick={onClose}
                            style={{fontSize:16,border:"1px solid #e5e7eb",padding:"4px 8px",borderRadius:8}}>닫기</button>
                </div>
                    ...
                    {sorted.map((ev,i)=>(
                        <li key={i} style={{border:"1px solid #e5e7eb",borderRadius:10,padding:10}}>
                            <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}>
                                <strong style={{fontSize:18}}>{ev.certificate?.trim() || "관련 자격증"}</strong>
                                <span style={{fontSize:14,padding:"2px 6px",borderRadius:99,border:"1px solid #e5e7eb"}}>
                  {TYPE_LABEL[ev.type]}
                </span>
                            </div>
                            <div style={{fontSize:14,color:"#374151"}}>{ev.startdate} ~ {ev.enddate}</div>
                        </li>
                    ))}
                </ul>
            </div>
