...
export const QuestionCard: React.FC<QuestionCardProps> = ({
    ...
    <div className={PracticeStyles.questionCard}>
        <div className={PracticeStyles.questionHeader}>문제 {number}</div>
        <p className={PracticeStyles.questionText}>{text}</p>
        <div className={PracticeStyles.choices}>
            {options.map((opt, idx) => (
                // idx: 보기(radio 버튼) 순번(0~3)
                <label key={idx} className={PracticeStyles.choiceItem}>
